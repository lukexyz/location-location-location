...
### P42 completion note (the carried backlog)

- The photo command takes `--prefer CANDIDATE_ID="Wikipedia page title"`, repeatable, for a place whose name finds the wrong article or none. The preview says the page is looked up as asked and not checked against coordinates; the lookup makes one title call instead of a name lookup and a geosearch; a preferred page that is missing, ambiguous, or without a free lead image is a note, never a substitute. The command refuses a spec that names a place outside the run. The skill's step 11 mentions it.
- The place card is anchored to its pin. The field planner now also reports where the selected pin sits and how big the field is; `cardPosition` puts the card to the pin's right when there is room and to its left otherwise, level with it, clamped clear of the side cards, the header row, and the bottom edge; a phone keeps the stylesheet's bottom sheet. While the map is in flight only the anchor follows, so the names are not replanned sixty times a second. The first browser measurement showed the flaw in the old fly-to: it centred the picked pin, and on a 1280-wide screen the hole between the side cards is 494 px, so a 360 px card had nowhere to go but over the pin. Picking a place now flies the pin to where its card fits (`pinTarget`), a little in from the rank panel and level with the middle of the hole, rather than dead centre. The contract test waits out the 0.7 s flight and then asserts the card is level with its pin and within a short gap of it.

## 2026-09-03 — Where the Project Stands After the Rip

- The product has now been run for real: four research runs on this Windows machine without a routing key, one ORR fetch, and one full rail, housing, and street-care import chain, all previewed, all within their caps, all loaded in the viewer from the local serve. Overpass answered five of seven calls first time; the other two were a 504 and a 429, resumed by rerunning.
- The public demo is three real towns with real evidence: fifteen measured OpenStreetMap observations, three HM Land Registry medians, three Defra fly-tipping rates, three ORR reliability figures, and three rail journeys that say they are estimates. Nothing in it is synthetic; the synthetic fixture lives on in `fixtures/synthetic/` for the tests that need its visit audits and small samples.
- The viewer plans its field: pins shrink to clear their neighbours, names yield to collisions and try both sides, the fit zoom goes to 14, and the place card sits beside its pin, which is flown to where the card fits.
