...
9. Check the final `provenance.json` and report cache use, unavailable evidence, confidence limits, and provider failures. Treat café reach as a 1.2 km straight-line proxy, not a walking-network isochrone.
10. Point the user to the final `results.json` for the viewer. Keep the run, cache, profile, origin, rail and housing inputs, budgets, and credentials private unless the user explicitly requests publication.
