...
| --- | --- |
| **Map** | A full-colour map with the search area ringed by a dotted purple line, numbered score pins with the place's name beside each where it fits, graded green from the run's worst fit to its best, and a key. OpenStreetMap basemap with visible attribution. |
...
| `fixtures/demo/` | The demonstration run: real, cited evidence for three towns, regenerated byte-for-byte by `scripts/build_demo.py`. |
...
(Welwyn Garden City, Hemel Hempstead, and Maidenhead) carrying real, cited
evidence from a real run: OpenStreetMap amenity counts along the walking
network, HM Land Registry flat sales, Defra fly-tipping rates, and ORR
punctuality. The rail journeys are agent estimates built from published
journey times and service patterns, and are labelled as estimates with
confidence capped at 0.5. The search boundary is hand-drawn for the demo.
...
- The public demo is real, cited evidence for three towns, and every fact carries its basis in the interface.
